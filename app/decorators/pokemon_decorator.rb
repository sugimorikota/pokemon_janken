class PokemonDecorator < Draper::Decorator
  ...
  def render_name_link
    if user_has_pokemon?
      h.link_to object.name, "#", onclick: "window.open('https://zukan.pokemon.co.jp/detail/#{object.no}', 'ポケモン詳細', 'width=600, height=550')"
    else
      "?"
    end
  end
  ...
  def render_type_images
    if user_has_pokemon?
      object.pokemon_types.map { |type| h.image_tag(type.type_image, height: '25', weight: '75') }.join.html_safe
    else
      object.pokemon_types.map { |type| h.image_tag('pokemonbooks_type.png', height: '25', weight: '75') }.join.html_safe
    end
